...
private char[][] matrix;
	...
	public boolean play(Player player, Move move) throws invaliMoveException {
		int i = move.getI();
		int j = move.getJ();
	
		if (i < 0 || j < 0 || i >= Constants.BOARD_SIZE || j >= Constants.BOARD_SIZE) {
			throw new invaliMoveException(" O intervalo da jogada é inválido");
		}
		
		if (matrix[i][j] != ' ') {
			throw new invaliMoveException(" Esta jogada já foi realizada");
		}
